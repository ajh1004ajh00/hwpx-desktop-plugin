...
현재 커서·선택 영역 입력, 표/문서 분석, 작업본 저장에 집중합니다.
...
플러그인을 통해 작업할 때 첨부 파일의 접근 가능한 로컬 경로가 있으면 에이전트가 이 선택을
바로 수행합니다. 파일이 열린 뒤 읽기·분석·지원되는 입력은 WebMCP 도구를 우선 사용합니다.
...
- Studio의 `파일 > 저장` 또는 `파일 > 다른 이름으로 저장`으로 HWPX 파일을 저장합니다.
