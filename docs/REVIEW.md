...
2. desktop/studio.mjs: editor 생성과 WebMCP 등록.
3. desktop/studio-tools.mjs: 다섯 개 WebMCP 도구와 스냅샷 보관.
...
- WebMCP가 지원하는 작업에 불필요한 화면 자동화를 사용하지 않는가?
...
표 안에서 읽기 및 분석 → 문서 분석 페이징 → Studio 파일 메뉴로 저장 → 다시 열기.
...
- `npm test`: 18 passed, 0 failed. 이전 필드 등록 방식 테스트는 포함하지 않는다.
- 플러그인 manifest validator 및 skill validator 통과.
- 실제 Desktop Built-in Browser, 별도 4176 포트에서 Studio 화면과 다섯 WebMCP 도구 확인.
- 바깥 파일 버튼 제거와 Studio 단일 파일 메뉴의 열기·저장 항목 확인. 재설치 후 새 작업에서의
  host 첨부 파일 자동 선택은 이번 검증에서 미확인이다.
...
- Studio 파일 메뉴를 통한 실제 디스크 저장 및 저장본 재열기는 이번 검증에서 미확인이다.
