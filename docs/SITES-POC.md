...
## 선택 영역 글자 서식 도구 — 2026-09-15

#30에서 `hwpx_studio_apply_char_format`을 추가했다. 공개 Sites 버전 9가 최종 배포됐으며 URL은 유지된다.

- Site 소스: `2cf2564970932a89399fbd03a51bdd66e4c1bcbd`. 저장 묶음 66개, 53,780,480바이트.
- WebMCP 도구 8개 발견. 새 도구 입력은 font_family, font_size_pt, bold, italic, underline, strikethrough, text_color다.
- 새 시험 탭의 일반 본문 둘째 문단에서 합성 문구 `서식도구검증둘`만 탐색·선택했다. 맑은 고딕 16pt, 굵게, 기울임, 밑줄, 취소선, 글자색 #1A2B3C를 한 호출로 적용했다.
- 적용 후 문서 분석에서 선택 문구와 일곱 서식 값이 모두 일치했다. 첫 문단과 선택 밖 텍스트는 함초롬바탕 10pt 및 기본 서식을 유지했다.
- Ctrl+Z 후 선택 문구가 함초롬바탕 10pt 및 기본 서식으로 복구됐고, Ctrl+Shift+Z 후 일곱 서식 값이 다시 적용됐다.
- 성공 응답은 command_id, status, saved만 반환했다. 선택 텍스트와 내부 상태는 응답에 포함하지 않았다.
- 빈 문서 첫 문단은 HWP 구역 컨트롤을 포함해 기존 보호 규칙상 `BODY_CONTROLS_UNSUPPORTED`다. 일반 본문 문단에서만 자동 서식 적용을 검증했다.

이는 합성 문서와 현재 Codex 내장 브라우저의 자동 검증이다. 실제 사용자 파일의 글꼴 렌더링·저장·재열기·PDF, 실제 한글 IME, 독립 ChatGPT 환경과 macOS 검증은 사람이 수행해야 한다.
