...
현재 배포 경로는 공개 Sites다. 2026-09-14 사용자가 URL을 가진 누구나 접근하도록 승인했고 공개 설정을 확인했다. Sites 이용자는 GitHub/npm/localhost 설치를 하지 않으며, 해당 다운로드 설명은 로컬 개발 경로에만 적용한다. 각 방문자의 문서는 각자의 브라우저에서 처리되고 다른 방문자에게 자동 공유되지 않는다.
...
이 문서는 구현 기준이며 공개 개인정보처리방침·약관을 대신하지 않는다. 공개 운영자명은 **AnJuHyun**, 지원 이메일은 [ajhajh503@gmail.com](mailto:ajhajh503@gmail.com), GitHub 계정은 `ajh1004ajh00`이다(2026-09-18 운영자 지정). [저장소 Issues](https://github.com/ajh1004ajh00/hwpx-desktop-plugin/issues)에서도 문제를 제보할 수 있다. 공개 안내는 [개인정보처리방침](PRIVACY.md), [이용약관](TERMS.md), [지원 안내](SUPPORT.md)를 따른다. 지원 환경은 제출 전 QA 결과로 확정한다.
...
| 사용자 편집기 자산 요청 | Sites에서 HTML·JS·WASM·글꼴 다운로드 | 브라우저 메모리/디스크 캐시. 보관 기간은 브라우저·host 정책에 따름 | 사용자 문서와 구분. 방문자는 Node/Git/npm/localhost 불필요 |
| 개발용 의존성 요청 | 개발자가 GitHub·npm에서 다운로드 | 개발 checkout의 .runtime/node_modules | 최종 사용자 설치 경로가 아님 |
...
```mermaid
flowchart LR
  A[Sites 정적 편집기 자산] --> C[각 사용자 브라우저 Studio]
  A --> H[호스팅 요청 로그: 보관 기간·지역 미확인]
  F[사용자가 선택한 원본 파일] --> C
  C --> D[로컬 문서 모델·Undo]
  D --> L[로컬 복구·이력 저장소]
...
Sites는 정적 편집기 자산을 제공하고 원본 문서는 브라우저에서 처리한다. 개발자 운영의 문서 업로드 서버는 없다. 이것은 플랫폼 운영 로그가 없거나 AI 제공 내용이 보관되지 않는다는 뜻은 아니다. 운영 로그·지역·민감정보 처리 가능 범위는 별도 미확인이다. 사이트 로그인, 플러그인 OAuth, 개발자 신원 인증은 각각 다른 결정이다.
...
2026-09-18 확인: 같은 사이트라도 브라우저 프로필·host가 다르면 저장 공간이 다를 수 있다. 반대로 같은 프로필·origin의 여러 탭은 IndexedDB를 공유할 수 있다. 사용자별 서버 계정 격리가 구현된 것으로 설명하지 않는다. PC 종료는 RAM을 비우지만 디스크 캐시·IndexedDB 삭제를 보장하지 않는다. 사이트 데이터 삭제, 다운로드 파일 삭제, ChatGPT 대화 삭제는 서로 별개다.
