...
1. 최신 한글메이트 플러그인을 설치하거나 업데이트한다.
2. 새 대화에서 **“한글메이트 열어줘”**라고 요청한다.
3. 파일 > 열기로 HWP/HWPX를 선택하거나 새 문서를 작성한다.
4. 수정할 부분을 선택하고 원하는 내용을 요청한다.
5. 결과를 확인한 뒤 파일 메뉴에서 저장한다.

플러그인을 설치하면 바로 사용할 수 있으며, 별도의 프로그램이나 서버를 준비할 필요가 없다.

공개 URL은 편집기를 공유한다. 서로 다른 방문자가 같은 문서를 실시간 공유하는 기능은 없다. 파일은 각자의 브라우저에서 처리하고 자동 복구본이 남을 수 있다. AI 요청에 필요한 내용은 ChatGPT에 제공된다.

파일 열기 창이 나타나면 사용할 문서를 직접 선택한다. 새 버전으로 업데이트한 뒤에는 새 대화를 시작한다.
