...
## 제출 후보 정리 (2026-09-18 추가)

후보 버전 `0.1.0+codex.20260918031814`는 정책·약관·지원 안내와 512px 로고를 포함한 11개 파일 ZIP이다. 기존 6개 파일 기록은 이전 후보에 대한 것이다. upstream 빌드 lock을 재현 가능한 패치로 고정한 뒤 production 및 upstream 전체 audit가 각각 0건이다. 복구 33개, 패치/ZIP 회귀 2개, 패키지 검사를 통과했다.

Windows 한글 2024 13.0.0.564에서 엔진 생성 F1 HWP/HWPX의 본문과 부분 서식을 확인했다. 실제 브라우저 다운로드 파일 비교는 아니다. 전체 로컬 검사 54개 중 53개 통과, 1개는 한글에서 열린 F1 HWPX의 EBUSY 잠금으로 실패했다. 최종 커밋의 깨끗한 CI 결과를 별도 기록한다.

Windows UI 제어는 현재 브라우저 URL을 확인하지 못해 안전 검사로 중단됐다. Windows 저장·PDF·실제 IME, 최종 설치, macOS QA와 개발자 인증은 완료 처리하지 않는다. [최종 QA 인수인계](SUBMISSION-QA.md)를 따른다.
